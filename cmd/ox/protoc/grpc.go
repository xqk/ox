...
func installGRPCGen() (err error) {
	...
	if gofastPath, err = exec.LookPath("protoc-gen-gofast"); err != nil {
		fmt.Println(ocolor.Green("start installing protoc-gen-gofast"))
		if err = executeGoGet(_genGoFastAddress); err != nil {
			return
		}
	}
	fmt.Println(ocolor.Green("protoc-gen-gofast installation was successful, the installation path is:", gofastPath))
	return
}
...
func protocEnvCheck() (err error) {
	...
	fmt.Println(ocolor.Green("Protoc environment monitoring is successful , the installation path is:", protocPath))
	return
}
func doGenerate() (err error) {
	...
	if genAbsPath, err := filepath.Abs(option.outputDir); err == nil {
		fmt.Println(ocolor.Green("pb.go file generated successfully. The path is as follows:", genAbsPath))
	}
	return
}
