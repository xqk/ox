...
func (y *yaseeDataSource) watch() {
	for {
		...
		// client get err
		if err != nil {
			olog.Error("yaseeDataSource", olog.String("listenConfig curl err", err.Error()))
			time.Sleep(time.Second * 1)
			continue
		}
		if resp.StatusCode() != 200 {
			olog.Error("yaseeDataSource", olog.String("listenConfig status err", resp.Status()))
			time.Sleep(time.Second * 1)
			continue
		}
		...
		if yaseeRes.Code != 200 {
			olog.Info("yaseeDataSource", olog.Int64("code", int64(yaseeRes.Code)))
			time.Sleep(time.Second * 1)
			continue
		}
...
func (y *yaseeDataSource) updateConfig(data string, version int64) {
	...
	case y.changed <- struct{}{}:
		...
		olog.Info("yaseeDataSource", olog.String("change", data))
	default:
	}
}
