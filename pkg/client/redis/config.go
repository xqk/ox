...
func DefaultRedisConfig() Config {
	return Config{
		...
		WriteTimeout:  otime.Duration("1s"),
		IdleTimeout:   otime.Duration("60s"),
		ReadOnly:      false,
		Debug:         false,
		EnableTrace:   false,
		SlowThreshold: otime.Duration("250ms"),
		OnDialError:   "panic",
...
func StdRedisConfig(name string) Config {
	return RawRedisConfig("ox.redis." + name)
}
...
func RawRedisConfig(key string) Config {
	...
	if err := conf.UnmarshalKey(key, &config); err != nil {
		olog.Panic("unmarshal redisConfig",
			olog.String("key", key),
			olog.Any("redisConfig", config),
			olog.String("error", err.Error()))
	}
	return config
}
...
func (config Config) Build() *Redis {
	...
	if count < 1 {
		config.logger.Panic("no address in redis config", olog.Any("config", config))
	}
...
func (config Config) buildStub() *redis.Client {
	...
	if err := stubClient.Ping().Err(); err != nil {
		...
		case "panic":
			config.logger.Panic("dial redis fail", olog.Any("err", err), olog.Any("config", config))
		default:
			config.logger.Error("dial redis fail", olog.Any("err", err), olog.Any("config", config))
		}
	}
...
func (config Config) buildCluster() *redis.ClusterClient {
	...
	if err := clusterClient.Ping().Err(); err != nil {
		switch config.OnDialError {
		case "panic":
			config.logger.Panic("start cluster redis", olog.Any("err", err))
		default:
			config.logger.Error("start cluster redis", olog.Any("err", err))
		}
	}
...
func StdRedisStubConfig(name string) Config {
	return RawRedisStubConfig("ox.redis." + name + ".stub")
}
...
func StdRedisClusterConfig(name string) Config {
	return RawRedisClusterConfig("ox.redis." + name + ".cluster")
}
