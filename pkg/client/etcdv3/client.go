...
func newClient(config *Config) (*Client, error) {
	...
	config.logger = config.logger.With(olog.FieldAddrAny(config.Endpoints))
	...
	if config.CaCert != "" {
		...
		if err != nil {
			config.logger.Panic("parse CaCert failed", olog.Any("err", err))
		}
	...
	if config.CertFile != "" && config.KeyFile != "" {
		...
		if err != nil {
			config.logger.Panic("load CertFile or KeyFile failed", olog.Any("config", config), olog.Any("err", err))
		}
	...
	if err != nil {
		// config.logger.Panic("client etcd start panic", olog.FieldMod(ecode.ModClientETCD), olog.FieldErrKind(ecode.ErrKindAny), olog.FieldErr(err), olog.FieldValueAny(config))
